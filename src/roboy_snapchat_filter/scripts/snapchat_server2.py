    ...
    while run_event.is_set(): #while the thread is active we loop
        ...
        for face in faces: #if there are faces
           ...
           # if (SPRITES[0] & SPRITES[1]):
            #    apply_sprite(image, "./sprites/roboy_moustache.png",w+35,x-15,y-40, incl, ontop = False)
                
             #  	if is_mouth_open:
              #      apply_sprite(image, "./sprites/doggy_tongue.png",w+35,x-15,y-40, incl, ontop = False)

           
			#roboy condition
            if SPRITES[0]:
                apply_sprite(image, "../sprites/roboy.png",w+35,x-15,y-40, incl, ontop = False)
            
                #if is_mouth_open:
                #	apply_sprite(image, "./sprites/rainbownew.png",w0,x0,y0+40, incl, ontop = False)
            
            if (is_mouth_open and SPRITES[0] ):
                apply_sprite(image, "../sprites/roboy_rainbow.png",w+35,x-15,y-40, incl, ontop = False)
            ...
            if (SPRITES[0] & SPRITES[1]):
                apply_sprite(image, "../sprites/roboy_mustache.png",w+35,x-15,y-40, incl, ontop = False)

            #pixelated sunglasses condition
            if SPRITES[2]:
                (x3,y3,_,h3) = get_face_boundbox(shape, 1)
                apply_sprite(image, "../sprites/pixelated_sunglasses.png",w,x,y3, incl, ontop = False)

            #roboy + sunglasses condition
            if (SPRITES[0] & SPRITES[2]):
                apply_sprite(image, "../sprites/roboy_sunglasses.png",w+35,x-15,y-40, incl, ontop = False)
			
            #roboy + mustache +sunglasses condition
            if (SPRITES[0] & SPRITES[1] & SPRITES[2]):
                apply_sprite(image, "../sprites/roboy_sunglasses_mustache.png",w+35,x-15,y-40, incl, ontop = False)
            ...
            #crown condition
            if SPRITES[6]:
                apply_sprite(image, "../sprites/crown.png",w+30,x-15,y, incl)
			...
			#glasses condition
               ...
               # apply_sprite(image, "../sprites/glasses.png",w,x,y3, incl, ontop = False)
                ...
                if (not SPRITES[0]) and is_mouth_open:
...
# Adds a custom logo
imgicon = PhotoImage(file=os.path.join(this_dir,'../sprites/roboy_sunglasses_mustache.png'))
root.tk.call('wm', 'iconphoto', root._w, imgicon)

##Create 5 buttons and assign their corresponding function to active sprites
btn1 = Button(root, text="Roboy", command = lambda: put_sprite(0))
btn1.pack(side="top", fill="both", expand="no", padx="5", pady="5")

btn2 = Button(root, text="Moustache", command = lambda: put_sprite(1))
...
btn4 = Button(root, text="Hat", command = lambda: put_sprite(3) )
btn4.pack(side="top", fill="both", expand="no", padx="5", pady="5")

btn5 = Button(root, text="Flies", command = lambda: put_sprite(4) )
...
btn7 = Button(root, text="Crown", command = lambda: put_sprite(6) )
btn7.pack(side="top", fill="both", expand="no", padx="5", pady="5")

#btn8 = Button(root, text="Roboy", command = lambda: put_sprite(7) )
#btn8.pack(side="top", fill="both", expand="no", padx="5", pady="5")
...
SPRITES = [0,0,0,0,0,0,0] #hat, mustache, flies, glasses, doggy -> 1 is visible, 0 is not visible
...
#     # Adds a custom logo
#     imgicon = PhotoImage(file=os.path.join(this_dir,'./sprites/roboy_sunglasses_moustache.png'))
#     root.tk.call('wm', 'iconphoto', root._w, imgicon)
